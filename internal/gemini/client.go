...
1. **찬송가 YouTube 임베딩 (필수 형식!)**
   ...
   - 키워드와 관련된 찬송가를 선택하되, 다양한 찬송가를 활용하세요:
     * "사랑" → 304, 364, 412, 220, 93, 279, 292, 404, 218, 199 중 선택
     * "기도" → 492, 363, 411, 365, 361, 362, 487, 490 중 선택 (305번 제외!)
     * "은혜" → 94, 310, 413, 200, 438, 287, 288, 406, 407, 410 중 선택
     * "찬양" → 31, 32, 33, 34, 67, 35, 36, 37, 38, 39 중 선택
     * "십자가" → 136, 147, 144, 151, 149, 135, 138, 139, 140, 141 중 선택
     * "감사" → 301, 309, 428, 429, 588, 589, 590, 592, 593, 594 중 선택
     * "소망" → 488, 491, 370, 495, 384, 380, 487, 498, 499, 500 중 선택
     * "믿음" → 338, 336, 357, 330, 342, 343, 344, 345, 346, 347 중 선택
     * "평안" → 413, 371, 377, 419, 478, 559, 560, 561, 562, 563 중 선택
     * "구원" → 254, 255, 256, 257, 258, 259, 260, 261, 262, 263 중 선택
     * "회개" → 268, 269, 270, 271, 272, 273, 274, 275, 276, 277 중 선택
     * "헌신" → 323, 324, 325, 326, 327, 328, 329, 321, 322, 315 중 선택
     * 기타 키워드는 관련성 있는 찬송가 자유롭게 선택
...
4. **찬송가 선택 가이드**
   - **⚠️ 매우 중요 - 다양성 필수**:
     ...
     * 예: "기도" → 오늘은 363번, 내일은 411번, 모레는 492번
   - YouTube 검색 시 "찬송가 [번호]장" 형식 사용
   - 실제 존재하는 찬송가 번호만 사용 (1-645장)
   - **절대 금지**: 매번 같은 번호(특히 305, 310, 94) 반복 사용
