...
def extract_hymn_number(content):
    """
    블로그 콘텐츠에서 찬송가 번호 추출

    Args:
        content: 블로그 마크다운 콘텐츠

    Returns:
        찬송가 번호 (int) 또는 None
    """
    # 패턴: "### 찬송가 123장" 또는 "**오늘의 찬송**: 123장"
    patterns = [
        r'### 찬송가 (\d+)장',
        r'\*\*오늘의 찬송\*\*:\s*(\d+)장',
        r'찬송가 (\d+)장',
...
def generate_blog_content(data):
    ...
    try:
        ...
        # 생성된 콘텐츠에서 사용된 찬송가 번호 추출
        hymn_number = extract_hymn_number(content.get('content', ''))
        if hymn_number:
            content['hymn_number'] = hymn_number
            print(f"   🎵 찬송가 {hymn_number}장 포함됨")
