...
✅ 5.9단계: 찬송가 검증 강화 & 자동 스케줄링 (완료) - 2025-10-19
...
- **랜덤 키워드 지원**: 키워드 비워두면 keywords 테이블에서 랜덤 선택 (2025-10-19 추가)
- **YouTube 임베딩**: 자동으로 관련 찬송가 영상 검색 및 임베딩
- **찬송가 제목 일치 검증**: 본문 찬송가 번호와 YouTube 임베딩 번호 일치 강제 (2025-10-19 강화)
...
- **자동 발행 조건**:
  ...
  - **찬송가 제목 일치 필수** (불일치 시 기술 점수 ≤3, 발행 불가) - 2025-10-19 강화
  - 치명적 문제 없음
...
#### 4. 자동 스케줄링 (2025-10-19 추가)
- **Cron 기반**: 매일 또는 원하는 시간에 자동 블로그 생성
- **systemd Timer**: 고급 스케줄링 옵션
- **Go 스케줄러**: 애플리케이션 내장 스케줄러 (선택)
- **자세한 가이드**: `docs/AUTO_SCHEDULING.md` 참조
...
POST /api/blogs/generate           # 블로그 자동 생성 (키워드 선택사항, 비우면 랜덤)
GET  /api/blogs/:id                # 블로그 조회
POST /api/blogs/:id/evaluate       # 수동 평가 저장
POST /api/blogs/:id/gemini-evaluate # Gemini 자동 평가
POST /api/blogs/:id/regenerate     # 블로그 재생성 (항상 가능)
POST /api/blogs/:id/publish        # 발행 토글
DELETE /api/blogs/:id              # 블로그 삭제
GET  /api/stats                    # 통계
GET  /api/blogs/:id/evaluation-history # 평가 이력
```

#### 블로그 자동 생성 API 사용법 (2025-10-19 업데이트)

```bash
# 키워드 지정 생성
curl -X POST http://localhost:9090/api/blogs/generate \
  -H "Content-Type: application/json" \
  -d '{"keyword": "사랑", "date": "2025-10-19"}'

# 랜덤 키워드 생성 (keywords 테이블에서 자동 선택)
curl -X POST http://localhost:9090/api/blogs/generate \
  -H "Content-Type: application/json" \
  -d '{"keyword": "", "date": "2025-10-19"}'
```
...
#### 자동 스케줄링 (2025-10-19 추가)

**Cron 설정 예시** (매일 오전 6시 블로그 자동 생성):

```bash
# 스크립트 생성
cat > /home/ec2-user/bibleai/scripts/auto_blog_generate.sh << 'EOF'
#!/bin/bash
TODAY=$(date +%Y-%m-%d)
curl -X POST http://localhost:9090/api/blogs/generate \
  -H "Content-Type: application/json" \
  -d "{\"keyword\": \"\", \"date\": \"$TODAY\"}" \
  >> /home/ec2-user/bibleai/logs/auto_blog.log 2>&1
EOF
...
# Cron 설정
crontab -e
# 추가: 0 6 * * * /home/ec2-user/bibleai/scripts/auto_blog_generate.sh
```
