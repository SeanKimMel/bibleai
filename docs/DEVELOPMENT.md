...
- **Git**: 최신 버전
- **PostgreSQL**: 16 이상 (로컬 개발용)

### 타임존 설정 (2025-10-23)
프로젝트는 한국 시간대(Asia/Seoul)를 기준으로 동작합니다:
- **DB 타임존**: Asia/Seoul (TIMESTAMPTZ 사용)
- **백엔드**: DB 연결 시 `timezone=Asia/Seoul` 설정
- **프론트엔드**: 한국 시간대로 명시적 변환하여 표시
- **목적**: 전 세계 어디서든 한국 시간 기준으로 일관된 표시
